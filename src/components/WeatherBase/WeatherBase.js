...
const WeatherBase = ({ min, max, dt, description, icon, temp, sunrise, sunset, feels_like, humidity, wind_speed, type }) => {
    ...
    return (
        ...
        <article className={type} >
                ...
                >
                    ...
                    {sunrise && sunset ?
                        ...
                        <div>
                            ...
                            <p><b>Sunrise:</b> {sunriseTime.toLocaleTimeString().replace('π.μ.', 'a.m.')}</p>
                            <p><b>Sunset:</b> {sunsetTime.toLocaleTimeString().replace('μ.μ.', 'p.m.')}</p>

                        </div>
                        :
                        null}

                    {feels_like ? <p><b>Feels like:</b> {Math.round(feels_like)}℃</p> : null}

                    <p><b>Wind:</b> {weatherWind(wind_speed)[0]} b., {weatherWind(wind_speed)[1]} </p>
            ...
            {type === 'current' ?
                <h1 className="header" >Weather in Athens</h1> : ""}
